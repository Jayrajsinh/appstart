...
class ModuleImageGallery_IndexController extends Zend_Controller_Action {
	...
	public function deleteAction() {
		...
		$this->_helper->viewRenderer->setNoRender ();
		$request = $this->getRequest ();
		if (($module_image_gallery_id = $request->getParam ( "id", "" )) != "") {
			$model = new ModuleImageGallery_Model_ModuleImageGallery ();
			if ($model) {
				try {
					$mapper = new ModuleImageGallery_Model_Mapper_ModuleImageGallery ();
					$mapper->getDbTable ()->getAdapter ()->beginTransaction ();
					$detailmapper = new ModuleImageGallery_Model_Mapper_ModuleImageGalleryDetail ();
					$data = $mapper->fetchAll ( "module_image_gallery_id = " . $module_image_gallery_id );
					if ($data) {
						foreach ( $data as $image ) {
							$dataDetails = $detailmapper->fetchAll ( "module_image_gallery_id =" . $image->getModuleImageGalleryId () );
	...
	public function gridAction() {
		...
		foreach ( $rows as $rowId => $row ) {
			$edit = array ();
			if ($row [4] ["migd.module_image_gallery_detail_id"] == "") {
				$mapper = new ModuleImageGallery_Model_Mapper_ModuleImageGalleryDetail ();
				$details = $mapper->fetchAll ( "module_image_gallery_id=" . $row [4] ["mig.module_image_gallery_id"] . " AND language_id=" . $default_lang_id );
				if (is_array ( $details )) {
					$details = $details [0];
			...
			if ($languages) {
				foreach ( $languages as $lang ) {
					$editUrl = $this->view->url ( array (
							...
							"controller" => "index",
							"action" => "edit",
							"id" => $row [4] ["mig.module_image_gallery_id"],
